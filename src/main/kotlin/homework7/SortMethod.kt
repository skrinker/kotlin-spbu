...
object MergeSort : SortMethod {
    ...
    private suspend fun IntArray.mergeSortParallel(lowerBound: Int, upperBound: Int, threads: Int) {
        if (lowerBound + 1 >= upperBound) return
        val mid = (lowerBound + upperBound) / 2

        coroutineScope {
            val lowerThread = launch { mergeSortParallel(lowerBound, mid, threads / 2) }
            val upperThread = launch { mergeSortParallel(mid, upperBound, threads - threads / 2) }
            lowerThread.join()
            upperThread.join()
        }
    ...
    private suspend fun IntArray.mergeArraysParallel(left: Int, right: Int, mid: Int) {
